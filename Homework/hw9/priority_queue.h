...
void PriorityQueue<T>::remove(T element) {
  ...
  //
  //checks to make sure the element you are planning to remove exsists
  if(locations.find(element) == locations.end()){
    return;
  }

  //sets a temp variable to the last item
  T temp = m_heap.back();
  //sets the index where you want to move the last item to wherever the element you are removing is
  int index= locations[element];
  //if it is the last element you don't need to switch anything so just delete it and delete the location
  if(index == m_heap.size()-1){
    ...
    //sets the heap index to the last value of list
    m_heap[index] = temp;
    //updates the location of the last value of the list to index
    locations[temp]= index;
    //erases the location of the element you are removing
    locations.erase(element);
    //delete the last item
    m_heap.pop_back();
    //percilate the value you just moved up and down until it settles in
    this->update_position(m_heap[index]);
  }
...
void PriorityQueue<T>::percolate_up(int i) {
  ...
  //if it is the top then do nothing  
  if(i==0){
    return;
  }
  ...
  //checks if the parent priority value is greater than the child, if that is the case then swith their
  //locations and percolate up
  if(m_heap[i]->getPriorityValue() < m_heap[parent]->getPriorityValue() ){
    
  
    //saves the two values that were in child and parent as p and child
    T child= m_heap[i];
    T p = m_heap[parent];

    
    //sets the child heap value and location to that of the parent
    m_heap[parent]= child;
    locations[child]= parent;
  
    //sets the parent heap value and location to that of the child
    m_heap[i]= p;
    locations[p]= i;
    
    //percolate up from the parent value
    percolate_up(parent);
  }else{
    return;
  }
...
void PriorityQueue<T>::percolate_down(int i) {
  ...
  //if it doesn't have a left child you're good
  if(!has_left_child(i)){
    return;
  }

  //sets the left and right child indexes
  int left_child = 2*i+1;
  int right_child = 2*i+2;
  //sets child to change to the current value you are on
  int childToChange= i;

  //switch with the smallest child

  //if the left child is less than the parent AND the left child is less than the right child than
  //set the child you should switch with to the left child
  if(m_heap[left_child]->getPriorityValue() < m_heap[i]->getPriorityValue() && 
    m_heap[left_child]->getPriorityValue()<m_heap[right_child]->getPriorityValue()){
    childToChange= left_child;
  }

  //if the right child exsists AND the parent is greater than the right child AND the right child
  //is less than the left child than switch with right child
  if(has_right_child(i) && m_heap[right_child]->getPriorityValue()<m_heap[i]->getPriorityValue()&&
    m_heap[left_child]->getPriorityValue()>m_heap[right_child]->getPriorityValue()){
    childToChange= right_child;
  }

  //if it found a valid child
  if(childToChange!=i){
    //switch the child and parent and percolate down from the new child you just switched to
    T tempEdge= m_heap[childToChange];
    m_heap[childToChange]=m_heap[i];
    m_heap[i]= tempEdge;
    
    locations[m_heap[childToChange]]= childToChange;
    locations[m_heap[i]]= i;
    percolate_down(childToChange);
  }
