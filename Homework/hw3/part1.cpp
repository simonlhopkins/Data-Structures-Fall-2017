...
void print_pace(int pace){
  
  float tempPace=(1/float(pace))*feet_per_mile;
  int min= int(tempPace);
  int seconds= (tempPace-min)*60;
  if(min<10){
    std::cout<<" "<<min;
  }
  else{
...
float getTotalDistance(GPSData data[], int n){
...
float distance(GPSData data[], int n, int &ave_feet_per_minute){
  int tempXValues[n];
  int tempYValues[n];
  for(int i=0; i<n; i++){
