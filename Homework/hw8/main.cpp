...
void student_tests() {
  ...
  //
  std::vector< std::pair<Point<int>,char> > points;
  //checks to see if it will work for an unbalanced tree
  points.push_back(std::make_pair(Point<int>(24, 24), 'A'));
  points.push_back(std::make_pair(Point<int>(22, 22), 'B'));
  points.push_back(std::make_pair(Point<int>(20, 20), 'C'));
  points.push_back(std::make_pair(Point<int>( 18, 18), 'D'));
  points.push_back(std::make_pair(Point<int>(16, 16), 'E'));
  points.push_back(std::make_pair(Point<int>(14, 14), 'F'));
  points.push_back(std::make_pair(Point<int>(12, 12), 'G'));
  points.push_back(std::make_pair(Point<int>( 10, 10), 'H'));
  points.push_back(std::make_pair(Point<int>(8, 8), 'I'));
  points.push_back(std::make_pair(Point<int>(6,6), 'J'));
  points.push_back(std::make_pair(Point<int>(4,4), 'K'));
  points.push_back(std::make_pair(Point<int>(2,2), 'L'));
  points.push_back(std::make_pair(Point<int>(0,0), 'M'));
  ...
  for (unsigned int i = 0; i < points.size(); i++) {
    ...
    tree1.insert(points[i].first,points[i].second);
  }
  tree1.plot(25, 25);
  tree1.print_sideways();
  assert (tree1.height()==12);

  //all in the same line
  QuadTree<int,char>* tree2 = new QuadTree<int,char>;
  tree2->insert( Point<int>(6,5),  'A');
  tree2->insert( Point<int>(6,7),  'X');
  tree2->insert( Point<int>(6,10),  'B');
  tree2->insert( Point<int>(6,13),  '&');
  tree2->insert( Point<int>(6,15),  'C');
  tree2->insert( Point<int>(6,20),  'D');
  tree2->plot(21, 21);
  tree2->print_sideways();
  assert(tree2->size()==6);
  std::pair<QuadTree<int,char>::iterator,bool> ret_val;
  ret_val=tree2->insert( Point<int>(6,20),  'D');
  assert(ret_val.second==false);
  QuadTree<int,char>::iterator treeItr;
  treeItr=tree2->begin();
  assert(treeItr.getLabel()=='A');
  treeItr++;
  tree2->insert( Point<int>(12,1),  'W');
  tree2->insert( Point<int>(16,7),  'N');
  tree2->insert( Point<int>(2,18),  'N');
  tree2->plot(21, 21);
  tree2->print_sideways();
  assert(treeItr.getLabel()=='X');
  //iterator goes out of bounds and will still just return NULL
  treeItr++;treeItr++;treeItr++;treeItr++;treeItr++;treeItr++;treeItr++;treeItr++;treeItr++;treeItr++;treeItr++;treeItr++;treeItr++;treeItr++;
  assert(treeItr==tree2->end());
  delete tree2;
  tree2=NULL;
  //tesing with floats!
  QuadTree<float,char>* floatTree = new QuadTree<float,char>;
  floatTree->insert( Point<float>(6.0,5.0),  '*');
  floatTree->insert( Point<float>(8.0,3.0),  '*');
  floatTree->insert( Point<float>(10.0, 12.0),  '*');
  floatTree->insert( Point<float>(22.0,22.0),  '*');
  floatTree->insert( Point<float>(23.0,26.0),  '*');
  floatTree->insert( Point<float>(29.0,25.0),  '*');
  floatTree->insert( Point<float>(18.0,14.0),  '*');
  floatTree->insert( Point<float>(21.0,1.0),  '*');
  floatTree->insert( Point<float>(12.0,28.0),  '*');
  floatTree->plot(30,30);

  QuadTree<float,char>* floatTree2(floatTree);
  floatTree2->insert(Point<float>(13.0,28.0),  '!');

  std::cout<<"float tree 2:"<<std::endl;

  floatTree2->plot(30, 30);
  delete floatTree;
  floatTree=NULL;

  QuadTree<int,char>* finalTree = new QuadTree<int,char>;
  finalTree->insert(Point<int>(10,10),  'A');
  finalTree->plot(20,20);
  QuadTree<int,char>::iterator fItr=finalTree->find(10, 10);
  fItr++;
  assert(fItr==finalTree->end());
  finalTree->insert(Point<int>(5,5),  'B');
  fItr= finalTree->find(10, 10);
  fItr++;
  assert(fItr!=finalTree->end());
  assert(fItr.getLabel()=='B');
  finalTree->plot(20,20);
