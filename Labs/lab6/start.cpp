...
int findPath(int startX, int startY, const vector<vector<bool> > &blocked_grid){
  if(startX<0 || startY<0){
    return 0;
  }
  if(blocked_grid[startX][startY]){
    return 0;
  }
  ...
  return (findPath(startX-1, startY, blocked_grid)+findPath(startX, startY-1, blocked_grid));
...
int main(int argc, char* argv[]) {
  ...
  std::cout<<findPath(start_x, start_y, blocked_grid)<<std::endl;
