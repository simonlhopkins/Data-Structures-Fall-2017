...
public:
  // CONSTRUCTOR & DESTRUCTOR
  BalloonSP(const std::string& name_) : name(name_) {
    std::cout << "BalloonSP constructor " << name << std::endl;
    num_ropes = 0;
    ropes = new dsSharedPtr<BalloonSP>[MAX_NUM_ROPES];
    
  }
  ~BalloonSP() {
    std::cout << "BalloonSP destructor " << name << std::endl;
    // don't try to destroy attached balloons, just delete array
    delete [] ropes;
  }
  ...
  const std::string& getName() const { return name; }
  int numRopes() const { return num_ropes; }
  const dsSharedPtr<BalloonSP> getRope(int i) const { return ropes[i]; }

  // print the balloons we are attached to
  void print() { 
    std::cout << "BalloonSP " << name << " is connected to: ";
    for (int i = 0; i < num_ropes; i++) {
      std::cout << ropes[i]->name << " ";
    }
    if (num_ropes == 0) std::cout << "nothing";
  ...
  // add a rope connecting this balloon to another
  void addRope(dsSharedPtr<BalloonSP> b) { 
    assert (num_ropes < MAX_NUM_ROPES);
    ropes[num_ropes] = b;
    num_ropes++;
  }
  // detach a rope connecting this balloon to another
  void removeRope(dsSharedPtr<BalloonSP> b) { 
    for (int i = 0; i < MAX_NUM_ROPES; i++) {
      if (ropes[i] == b) { ropes[i] = ropes[num_ropes-1]; num_ropes--; break; }
    }
  }
...
private:
  ...
  int num_ropes;
  // a dynamically allocated C-style array of ropes connecting to other Balloons
  dsSharedPtr<BalloonSP> *ropes; 
};

// ====================================================
// ====================================================
void deleteAll(Balloon* root, std::set<Balloon*> &balloonsToDelete){
    
  for(int i=0; i<root->numRopes();i++){
    if(balloonsToDelete.find(root->getRope(i))==balloonsToDelete.end()){
      balloonsToDelete.insert(root->getRope(i));
      deleteAll(root->getRope(i), balloonsToDelete);
    }
...
int main() {
  ...
  dsSharedPtr<BalloonSP> georgette(new BalloonSP("Mr Potato Head"));
  dsSharedPtr<BalloonSP> henry(new BalloonSP("Snoopy"));

  georgette->addRope(henry);
  henry = new BalloonSP("Tigger");
  georgette->addRope(henry);
  georgette->print();
  henry->print();
  
  dsSharedPtr<BalloonSP> isabelle(new BalloonSP("Shrek"));
  henry->addRope(isabelle);
  isabelle = new BalloonSP("Barney the Purple Dinosaur");
