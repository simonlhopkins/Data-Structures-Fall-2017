...
void hash_table_test(const std::string* input_data, int input_count, const std::string &operation, std::string *output_data, int &output_count) {
  ...
  if (operation == "sort") {
    std::cerr<<"Hash tables can't be sorted without them moving to an exterior data structure"<<std::endl;
    exit(0);
  } else if (operation == "remove_dups") {
    for(std::unordered_set<std::string>::iterator itr= ht.begin(); itr!= ht.end(); itr++){
      for(std::unordered_set<std::string>::iterator itr2 = ht.begin(); itr2 != ht.end(); itr2++){
    ...
    std::string mode;
    std::unordered_set<std::string>::iterator max= ht.begin();
    for(std::unordered_set<std::string>::iterator itr= ht.begin(); itr!= ht.end(); itr++){
      if(ht.count(*itr)> ht.count(*max)){
        max=itr;
      }
    }
    mode= *(max);
